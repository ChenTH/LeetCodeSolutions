...
//    publci static Set<String> used=new HashSet<>();
    ...
    public static void role1Calculate(List<UnilateralLine> lineList,List<String> result,Set<String> used){//找出第一种解决办法并解决
        ...
        for(String line:map.keySet()){
            for(UnilateralLine u: map.get(line)){
                if(!used.contains(u.getId())){
                    List<UnilateralLine> list=map.get(u.getECen()+','+u.getSCen());//找到返回的线路列表
                    if(list!=null){
                            if(u.getTType().equals("9.6m")){
                                for(UnilateralLine ret:list){
                                    if(!used.contains(ret.getId())){
                                        if(ret.getTType().equals("9.6m")){//如果找到了9.6的返回
                                            used.add(u.getId());
                                            used.add(ret.getId());
                                ...
                                for(UnilateralLine ret:list){
                                    if(used.contains(ret.getId())){
                                        if(ret.getTType().equals("9.6m")){//如果找到了两个9.6返回
    ...
    public static void role3Calculate(List<UnilateralLine> lineList,List<String> result,Set<String> used){//找出第二种解决办法并解决
        //todo 未完成
        HashMap<String,List<UnilateralLine>> map=new HashMap<>();
        for(UnilateralLine u:lineList){
            List<UnilateralLine> list=map.getOrDefault(u.getSCen()+','+u.getECen(),new LinkedList<>());
